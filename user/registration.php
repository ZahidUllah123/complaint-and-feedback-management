...
if(isset($_POST['submit'])){
    ...
    $gender1= mysqli_real_escape_string($cc,htmlentities(trim(strip_tags($_POST['gender']))));

    $token= bin2hex(random_bytes(15));
    
 $khan=mysqli_query($cc,"SELECT * FROM user_registration where email='$email'");
 $abc=mysqli_fetch_array($khan);

if($abc){

    echo" <script> alert('This Email Is Already Exsist');</script>";
}
else{


    $data= mysqli_query($cc,"SELECT * from student_data where f_name='$fname' and email='$email'");
    $getdata=mysqli_fetch_Array($data);

    if ( $getdata ) {
        $result=   mysqli_query($cc,"INSERT INTO user_registration (f_name , l_name , email , password , repassword  , gender , token )
    VALUE('$fname','$lname','$email','$pass','$repass','$gender1' ,'$token')");
    echo" <script> alert('Successfully Added');</script>";
    }
else{
    echo" <script> alert('This Student is not enrolled');</script>";
